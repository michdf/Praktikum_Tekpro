    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        while (sc.hasNext()) {
            ...
            if (employeeType.equals("Manager")) {
                ...
                double salesDone = sc.nextDouble(); 
                Admin admin = new Admin( numberOfHoursWorked, hourlyRate, storeDetails, basePay, empName,salesDone);
                AdminList.add(admin); 
            }
        ...
        for (Manager manager : managerList) {
            ...
            netPayment = manager.calculateNetPayment(manager.calculatePay());
            manager.setNetPayment(netPayment);
            System.out.println("Net Payment: $" + String.format("%.1f", manager.getNetPayment()));
            pwd.println("Net Payment: $" + String.format("%.1f", manager.getNetPayment()));

            double tax = manager.calculateTax(manager.calculatePay());
            ...
            System.out.print("Is " + manager.getEmployeeName() + " Fired? ");
            if ( manager.checkPromotionEligibility() && !manager.checkFiredEligibility()) {
                System.out.println("No, he/she isn't gonna be fired, he/she stay\n");
            } else if (!manager.checkPromotionEligibility() && manager.checkFiredEligibility()) {
                System.out.println("Yes, he/she is gonna be fired next month\n");
            } else if (!manager.checkPromotionEligibility() && !manager.checkFiredEligibility()) {
                System.out.println("No, he/she isn't gonna be fired, he/she stay\n");
            } else if (manager.checkPromotionEligibility() && manager.checkFiredEligibility()) {
                System.out.println("No, he/she isn't gonna be fired, he/she stay\n");
        ...
        for (SalesAssociate salesAssociate : SalesAssociateList) {
            ...
            netPayment = salesAssociate.calculateNetPayment(salesAssociate.calculatePay());
            salesAssociate.setNetPayment(netPayment);
            System.out.println("Net Payment: $" + String.format("%.1f", salesAssociate.getNetPayment()));
            pwd.println("Net Payment: $" + String.format("%.1f", salesAssociate.getNetPayment()));

            double tax = salesAssociate.calculateTax(salesAssociate.calculatePay());
            System.out.println("Tax to be paid: $" + tax);
            ...
            System.out.print("Is " + salesAssociate.getEmployeeName() + " Fired? ");
            if ( salesAssociate.checkPromotionEligibility() && !salesAssociate.checkFiredEligibility()) {
                System.out.println("No, he/she isn't gonna be fired, he/she stay\n");
            } else if (!salesAssociate.checkPromotionEligibility() && salesAssociate.checkFiredEligibility()) {
                System.out.println("Yes, he/she is gonna be fired next month\n");
            } else if (!salesAssociate.checkPromotionEligibility() && !salesAssociate.checkFiredEligibility()) {
                System.out.println("No, he/she isn't gonna be fired, he/she stay\n");
            } else if (salesAssociate.checkPromotionEligibility() && salesAssociate.checkFiredEligibility()) {
        ...
        System.out.println("******************************************************");
        System.out.println("Number of employees working as ADMIN are: " + AdminList.size());
        System.out.println("******************************************************");
        pwd.println("******************************************************");
        pwd.println("Number of employees working as ADMIN are: " + AdminList.size());
        pwd.println("******************************************************");

        for (Admin admin : AdminList) {
            adminCount++;
            System.out.println(adminCount + ". Admin Details : ");
            System.out.println(admin);
            pwd.println(admin);
            System.out.println("Total commission: $" + admin.calculateCommission());
            System.out.println("Gross Payment: $" + admin.calculatePay());

            pwd.println("Total commission: $" + admin.calculateCommission());
            pwd.println("Gross Payment: $" + admin.calculatePay());

            netPayment = admin.calculateNetPayment(admin.calculatePay());
            admin.setNetPayment(netPayment);
            System.out.println("Net Payment: $" + String.format("%.1f", admin.getNetPayment()));
            pwd.println("Net Payment: $" + String.format("%.1f", admin.getNetPayment()));

            double tax = admin.calculateTax(admin.calculatePay());
            System.out.println("Tax to be paid: $" + tax);

            System.out.print("Is " + admin.getEmployeeName() + " eligible for promotion? ");
            if (admin.checkPromotionEligibility() == true) {
                System.out.println("Yes, he/she is eligible\n");
            } else {
                System.out.println("No, he/she needs to work harder\n");
            }

            System.out.print("Is " + admin.getEmployeeName() + " Fired? ");
            if ( admin.checkPromotionEligibility() && !admin.checkFiredEligibility()) {
                System.out.println("No, he/she isn't gonna be fired, he/she stay\n");
            } else if (!admin.checkPromotionEligibility() && admin.checkFiredEligibility()) {
                System.out.println("Yes, he/she is gonna be fired next month\n");
            } else if (!admin.checkPromotionEligibility() && !admin.checkFiredEligibility()) {
                System.out.println("No, he/she isn't gonna be fired, he/she stay\n");
            } else if (admin.checkPromotionEligibility() && admin.checkFiredEligibility()) {
